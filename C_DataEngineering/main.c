    ...
    */
    ...
    for (int i = 1; i <= strlen(s1); i++)
        for (int j = 1; j <= strlen(s2); j++){
            prev[i][j]=min(array[i-1][j]+1, array[i-1][j-1] + (s1[i-1] != s2[j-1]), array[i][j-1]+1);
            if (prev[i][j]==1)
                array[i][j] = array[i-1][j]+1;
            else if(prev[i][j]==2)
                array[i][j] = array[i-1][j-1] + (s1[i-1] != s2[j-1]);
            else
